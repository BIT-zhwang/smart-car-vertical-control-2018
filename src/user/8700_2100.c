...
/******         读取2100陀螺仪数据       ******/
void Read_2100(unsigned char slave,unsigned char * a,short *b)
{
  int MUP_Zero=0x0000;
  a[0]= Read_Byte_2100_8700(slave,OUT_X_MSB_REG);
  a[1]= Read_Byte_2100_8700(slave,OUT_X_LSB_REG);
  a[2]= Read_Byte_2100_8700(slave,OUT_Y_MSB_REG);
  a[3]= Read_Byte_2100_8700(slave,OUT_Y_LSB_REG);
  
  //合并高低位
  b[0] = (((MUP_Zero | a[0])<<8)|a[1]);
  b[1] = (((MUP_Zero | a[2])<<8)|a[3]);
}
/******         done       ******/

/******         读取8700加速度计数据       ******/
void Read_8700(unsigned char slave,unsigned char * i,short *j)
{
  int MUP_Zero=0x0000;
  i[4]= Read_Byte_2100_8700(slave,OUT_Z_MSB_REG);
  i[5]= Read_Byte_2100_8700(slave,OUT_Z_LSB_REG);

  //合并高低位
  j[2] = (((MUP_Zero | i[4])<<8)|i[5]);
